...
@Component({
  ...
  templateUrl: 'note-hour-tab.html'
})
export class NoteHourTab {
  notes = []
  skip = 0
  limit = 10
  ...
  toMarkdown(note) {
    this.rootNavCtrl.push(MarkdownPage, {markdown: note.attributes.text})
  }
  ...
  toEdit(note) {
    this.rootNavCtrl.push(NoteEditPage, {
      note,
  ...
  createDaysMarkdown(notes) {
    ...
    this.rootNavCtrl.push(MarkdownPage, {markdown: markdown})
  }
